...
// ── Types ─────────────────────────────────────────────────────────
type Hero = {
  id: string; name: string; spritePath: string; level: number
  hp: number; attack: number; defense: number
  attackCooldown: number; walkSpeed: number; groupPosition: number; isActive: boolean
}
type Monster = {
  id: string; name: string; spritePath: string
  hp: number; attack: number; defense: number
  attackCooldown: number; walkSpeed: number; coinDropMin: number; coinDropMax: number
}
...
const HERO_DEFAULTS: Omit<Hero, 'id'> = {
  name: '', spritePath: 'hero', level: 1, hp: 26, attack: 4,
  defense: 0, attackCooldown: 1.5, walkSpeed: 220, groupPosition: 0, isActive: false,
}
        ...
        <table className="w-full text-xs mt-3">
          <thead>
            <tr className="text-gray-500 border-b border-gray-800">
              {['Nome','Nível','HP','ATK','DEF','Cooldown ATK','Vel.','Pos. Grupo','Ativo',''].map(h => (
          ...
          <tbody>
            {heroes.map(h => (
              <tr key={h.id} className="border-b border-gray-800/50 hover:bg-gray-900/50">
                ...
                <td className="pr-3">{h.walkSpeed}</td>
                <td className="pr-3">{h.groupPosition}</td>
                <td className="pr-3">{h.isActive ? <Pill color="green">sim</Pill> : <Pill color="gray">não</Pill>}</td>
      ...
      {editing && (
        <div className="w-72 shrink-0">
          <FormCard title={editing.id ? 'Editar Herói' : 'Novo Herói'} onCancel={() => setEditing(null)}>
            ...
            <Field label="Sprite path"><Input v={editing.spritePath ?? 'hero'} onChange={v => setEditing(e => ({ ...e!, spritePath: v }))} /></Field>
            <Row>
              <Field label="Nível"><NumInput v={editing.level ?? 1} onChange={v => setEditing(e => ({ ...e!, level: v }))} /></Field>
            ...
            <Row>
              <Field label="Pos. Grupo"><NumInput v={editing.groupPosition ?? 0} onChange={v => setEditing(e => ({ ...e!, groupPosition: v }))} /></Field>
              <Field label="Ativo">
                <select
                  value={editing.isActive ? '1' : '0'}
                  onChange={e => setEditing(prev => ({ ...prev!, isActive: e.target.value === '1' }))}
...
const MONSTER_DEFAULTS: Omit<Monster, 'id'> = {
  name: '', spritePath: 'zombie', hp: 12, attack: 1, defense: 0,
  attackCooldown: 2.2, walkSpeed: 42, coinDropMin: 1, coinDropMax: 2,
}
    ...
    <div className="flex gap-6">
      <div className="flex-1">
        ...
        <table className="w-full text-xs mt-3">
          <thead>
            <tr className="text-gray-500 border-b border-gray-800">
              {['Nome','HP','ATK','DEF','Cooldown ATK','Vel.','Moedas',''].map(h => (
                <th key={h} className="text-left py-2 pr-3 font-normal">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {monsters.map(m => (
              <tr key={m.id} className="border-b border-gray-800/50 hover:bg-gray-900/50">
                ...
                <td className="pr-3">{m.walkSpeed}</td>
      ...
      {editing && (
        <div className="w-72 shrink-0">
          <FormCard title={editing.id ? 'Editar Monstro' : 'Novo Monstro'} onCancel={() => setEditing(null)}>
            <Field label="Nome"><Input v={editing.name ?? ''} onChange={v => setEditing(e => ({ ...e!, name: v }))} /></Field>
            <Field label="Sprite path"><Input v={editing.spritePath ?? 'zombie'} onChange={v => setEditing(e => ({ ...e!, spritePath: v }))} /></Field>
            <Row>
